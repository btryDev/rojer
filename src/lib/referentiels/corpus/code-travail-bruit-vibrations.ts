...
export const CODE_TRAVAIL_BRUIT_VIBRATIONS: Corpus = {
  ...
  articles: [
    {
      ref: "R. 4431-2",
      intitule: "Valeurs limites d'exposition et valeurs déclenchant l'action de prévention",
      url: ART("LEGIARTI000018530386"),
      versionEnVigueur: "2008-05-01",
      modifiePar: null,
      luLe: "2026-09-21",
      lecture: "agent_verbatim",
      prescrit:
        "Fixe trois paliers d'exposition quotidienne au bruit : 87 dB(A) ou 140 dB(C) de crête (valeurs limites), 85 dB(A) ou 137 dB(C) (valeurs supérieures déclenchant l'action), 80 dB(A) ou 135 dB(C) (valeurs inférieures déclenchant l'action).",
      citationCle:
      ...
      statut: "sans_objet",
      motif:
        "Article de seuils : il ne prescrit rien par lui-même, il donne les trois paliers auxquels le reste du titre accroche ses obligations. OUVERT LE 2026-09-21 parce que deux entrées de ce corpus (`R. 4433-2`, `R. 4434-9`) étaient classées « texte à lire » en le nommant. Ce que la lecture établit : ces seuils sont des NIVEAUX MESURÉS, en décibels. Le produit ne détient aucun niveau de bruit — `Risque.dateMesuresPhysiques` est une date, sans grandeur ni facteur. Le blocage des deux entrées n'était donc pas un texte non lu, c'est une donnée que le module DUERP ne recueille pas ; leur `cause` passe à `module`. Aucun de ces chiffres n'est affiché au dirigeant.",
    },
    {
      ref: "R. 4432-1",
    ...
    },
    {
      ...
      toucheLaCible: true,
      motif:
        "[2026-09-21 — `R. 4431-2` EST OUVERT, et la cause change : ~~texte à lire~~ → `module`. Le renouvellement n'est dû qu'« en cas de mesurage », et le seul indice qu'en ait le produit est `Risque.dateMesuresPhysiques` — une date commune au bruit, à l'éclairement, aux ambiances thermiques et aux vibrations, sans dire laquelle de ces grandeurs a été mesurée. Ce qui manque est un champ du DUERP, pas une lecture.] LA SEULE PÉRIODICITÉ CHIFFRÉE DES DEUX TITRES, ET LE PRODUIT DÉTIENT DÉJÀ LA DATE QUI LA CALCULERAIT. `Risque.dateMesuresPhysiques` est saisie sur l'écran de cotation du DUERP, sous une aide qui nomme le bruit, l'éclairement, les ambiances thermiques et les vibrations, et elle est réimprimée dans le PDF. Elle n'est rattachée à aucune exigence : ~~personne ne dit au dirigeant que ce mesurage se renouvelle.~~ [rayé le 2026-09-26 : devenu faux, voir la note datée de ce motif] `quinquennale` existe au modèle, `personne_qualifiee` porte la compétence exigée, et le porteur établissement conviendrait.\n\nCE QUI LE BLOQUE EST UNE CONDITION D'ENTRÉE, ET ELLE EST RÉELLE. L'obligation ne naît que « EN CAS DE MESURAGE » — c'est-à-dire une fois que le « si nécessaire » de R. 4433-1 s'est déclenché, ce qui dépend des valeurs d'exposition de R. 4431-2, ~~article non ouvert~~ [ouvert le 2026-09-21, voir la note en tête de ce motif] et donnée que le produit ne détient pas. Faire naître une échéance quinquennale du seul fait qu'une date a été saisie inverserait la charge : ce serait n'exiger le renouvellement que de celui qui a déjà mesuré une fois, et rien de celui qui aurait dû. C'est très exactement le faux négatif d'ancrage corrigé sur R. 4227-34 le 2026-08-31, et l'y refaire ici serait le refaire en connaissance de cause.\n\n~~AJOUTÉ SANS ÊTRE CITÉ NULLE PART : cet article n'apparaît sur aucune surface.~~ [rayé le 2026-09-26 : devenu faux, voir la note datée de ce motif] Il entre au corpus parce qu'on a ouvert le chapitre voisin de celui que le PDF cite, et il est la seule chose que ce détour ait rendue. [2026-09-26 — le dernier alinéa (« En cas de mesurage, celui-ci est renouvelé au moins tous les cinq ans. ») est cité dans la description des trois risques bruit des référentiels sectoriels (`resto-bruit`, `com-bruit`, `bur-bruit-openspace`), et l'aide du champ « Dernières mesures physiques » ne dit plus qu'une mesure se fait « par un organisme habilité » — le texte dit « personnes compétentes ». Statut inchangé (E1, E5).]",
    },
    {
      ref: "R. 4434-9",
      ...
      toucheLaCible: true,
      motif:
        "[2026-09-21 — `R. 4431-2` EST OUVERT, et la cause change : ~~texte à lire~~ → `module`. ~~Le déclenchement suppose un niveau MESURÉ supérieur aux paliers de cet article ; le produit ne recueille aucun niveau de bruit.~~ [rayé le 2026-09-26 : non établi par ce qui a été lu — R. 4434-1 à -8 non ouverts] Même manque que `R. 4433-2`.] UN ACTE DE VÉRIFICATION QUE PERSONNE NE RÉCLAME, et c'est bien une obligation d'employeur — pas une définition, pas un renvoi. Elle ferme le chapitre IV comme une boucle : après avoir pris des mesures, en vérifier l'effet.\n\nPOURQUOI `obligation_manquante` PLUTÔT QUE `non_couvert`, puisque le domaine du bruit n'est effectivement pas servi. `non_couvert` dit « on a choisi de ne pas le porter, et on le dit à l'utilisateur ». Ni l'une ni l'autre moitié n'est vraie ici : aucune décision n'a jamais été prise sur ce domaine — l'inventaire de la partie IV le range comme « jamais ouvert » —, et aucune surface ne l'annonce à qui que ce soit. Surtout, les manques `non_couvert` du référentiel visent des établissements que le produit NE SERT PAS ; celui-ci vise un restaurant avec musique et lave-vaisselle, c'est-à-dire la cible même. Le classer là-bas ferait passer une dette pour une non-question, ce que `perimetre.ts` interdit en toutes lettres.\n\nCE QUI LE BLOQUE : aucun rythme dans le texte, et surtout aucun objet à vérifier tant que le produit ne sait pas quelles mesures ont été prises. ~~Son déclenchement suppose un dépassement des valeurs de R. 4431-2, donc un mesurage — la même condition d'entrée que R. 4433-2, et le même trou.~~ [rayé le 2026-09-26 : non établi par ce qui a été lu — R. 4434-1 à -8 non ouverts] [2026-09-26 — PAS annoncé, à dessein : l'article vise « les mesures prises en application du présent chapitre », et les articles R. 4434-1 à -8 qui disent lesquelles n'ont pas été lus ; la condition de dépassement que ce motif affirme n'est pas établie par ce qui a été lu. À lire avant de le dire.]",
    },
    {
      ref: "R. 4441-1",
