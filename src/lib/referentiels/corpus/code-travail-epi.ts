...
export const CODE_TRAVAIL_EPI: Corpus = {
  ...
  articles: [
    ...
    },
    {
      ...
      motif:
        "UN ÉCRIT QUE L'EMPLOYEUR DOIT DÉTENIR, QUE RIEN NE PORTE — ni le référentiel de conformité, ni `DOCUMENTS_OBLIGATOIRES`. Il a la forme que le produit sait déjà porter : porteur ÉTABLISSEMENT (la consigne décrit les équipements d'un lieu, pas la formation d'une personne), nature `etat_permanent` au sens de l'ADR-026 — à constituer une fois, à tenir à jour quand le parc change —, `periodicite: \"autre\"` puisque le texte ne chiffre rien, `realisateurs: [\"exploitant\"]` puisque personne ne la délivre. Exactement le patron de `code-travail-travail-en-hauteur` pour la notice des points d'ancrage de R. 4323-61. À la différence de la vérification périodique, il n'y a AUCUNE question d'assiette : la consigne est due dès qu'un travailleur doit utiliser un équipement de protection individuelle, quel qu'il soit — gants compris — et la question `epiPresents` de la fiche établissement collecte déjà la réponse qui la déclencherait. C'est le manque le plus directement encodable de tout ce lot.",
      bloquePar:
        "Rien de technique, et c'est ce qui le distingue de l'article 1er de l'arrêté : la scission de la catégorie `EPI` n'est PAS un préalable, puisque cette obligation ne se porte pas par équipement mais par établissement. Ce qui manque est l'encodage lui-même, plus une décision de périmètre qui n'appartient pas à un lot de dépouillement : la seconde pièce de l'article — « une documentation relative à la réglementation applicable » — est une documentation réglementaire tenue à disposition du CSE, et le produit n'a pas tranché s'il porte ce genre de pièce. La première (la consigne) ne pose pas cette question.",
    },
    {
      ref: "R. 4323-106",
