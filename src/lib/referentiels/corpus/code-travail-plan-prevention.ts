...
export const CODE_TRAVAIL_PLAN_PREVENTION: Corpus = {
  ...
  articles: [
    ...
    },
    {
      ...
      statut: "sans_objet",
      motif:
        "Article de CONTENU de l'inspection que R. 4512-2 impose, et non un acte distinct : il énumère les quatre gestes du chef d'établissement pendant la visite, tous accomplis le même jour, dans le même déplacement.\n\nCE MOTIF A EU DEUX FONDEMENTS FAUX AVANT CELUI-CI, ET LES DEUX MÉRITENT D'ÊTRE ÉCRITS. Le premier, du 2026-09-02, renvoyait le manque ailleurs : ce que le module ne collecte pas — la délimitation du secteur, le balisage des zones — était « le même écart de contenu que porte l'entrée `obligation_manquante` de R. 4512-8 ». L'argument tenait tant que R. 4512-8 comptait un manque ; il ne le compte plus depuis que le module sert ses cinq rubriques, et le renvoi pointait vers une case vide. Le second, écrit le 2026-09-07 pour le remplacer, alignait cet article sur R. 4512-4 et R. 4512-5 — « ni écrit, ni date, ni conservation, donc rien à détenir ». CE DÉPÔT A DÉJÀ TRANCHÉ L'INVERSE PAR ÉCRIT : `signalisation-etablissement-obstacles-zones-dangereuses` est encodée avec `pieceAttendue: null` et la note « la bande peinte ou collée EST l'obligation ». Une matérialisation sans écrit ni date est donc parfaitement encodable ici, et la même note NOMME celle que R. 4224-20 exige comme n'étant portée par aucune ligne — quand une matérialisation manque, ce dépôt la désigne, il ne la range pas dans une classe « rien à détenir ». Le rapprochement avec R. 4512-4 et -5 était doublement bancal : ces deux-là sont des communications dont l'objet même est indéterminé (« toutes informations nécessaires »), là où cet article énumère quatre gestes déterminés dont l'un laisse un état physique persistant.\n\nCE QUI FONDE RÉELLEMENT LE CLASSEMENT, ET VAUT IDENTIQUEMENT POUR LES QUATRE — donc justifie de les tenir en bloc : les quatre gestes portent sur LE SECTEUR D'UNE OPÉRATION DONNÉE, et le texte n'en fixe l'objet ni d'avance ni durablement. Quelles zones sont dangereuses, quelles voies sont empruntables, quels accès sont ouverts se détermine À L'INSPECTION, cas par cas, par les deux employeurs, et ne survit pas à l'opération. Le référentiel ne peut donc rien énumérer d'avance, et l'établissement n'a rien à retenir entre deux opérations. C'est la différence exacte avec le marquage des obstacles de l'arrêté du 4 novembre 1993, encodé PARCE QUE le texte en fixe l'objet — des bandes jaune et noir sur les obstacles de l'établissement, qui sont les mêmes demain qu'aujourd'hui. Le parallèle se prolonge d'ailleurs : le marquage des VOIES DE CIRCULATION de l'article 13 du même arrêté — le 3° d'ici — reste `obligation_manquante` au référentiel, sa condition d'entrée n'étant pas dépouillée.\n\nET NON, LE 2° NE SE LOGE PAS DANS `moyensPrevention`, même si un balisage est un moyen de prévention — c'est la première objection qu'on formera depuis le 2026-09-07. `PhaseDangereuse` est bornée par le texte du 1° de R. 4512-8 : des phases DE L'OPÉRATION et les moyens qui leur CORRESPONDENT. Le 2° d'ici est un geste de l'entreprise UTILISATRICE, accompli au cours de l\'inspection, sur le secteur pris globalement — pas par phase, pas par le même acteur, pas au même moment. L\'y ranger referait le « faire porter à une rubrique le contenu d\'une autre » que ce lot a refusé pour les lignes de R. 4512-6. Qu\'un champ de texte libre puisse accueillir une information ne fait pas de lui une couverture : `naturesTravaux` et `inspectionParticipants` le pourraient aussi.",
    },
    {
      ref: "R. 4512-4",
    ...
    },
    {
      ...
      toucheLaCible: true,
      motif:
        "LE 2° EST UNE DÉMARCHE À FAIRE, PAS UN DOCUMENT À RANGER, et c'est le seul acte du chapitre qui sorte de l'entreprise : informer par écrit l'inspection du travail de l'ouverture des travaux, à la charge du chef de l'entreprise utilisatrice, dès que l'écrit est obligatoire. ~~Rien dans le produit ne le nomme — ni le formulaire, ni la fiche de plan, ni le ZIP de contrôle —,~~ [2026-09-26 : les trois le nomment désormais, voir la fin de ce motif] alors que DEUX surfaces affichent la pastille « Art. R. 4512-6 à R. 4512-12 CT » et que la prose de la pastille parle de l'établissement conjoint du plan sans dire un mot de cette démarche. Un dirigeant qui lit l'écran conclut qu'il a fini quand il a signé. Le 1° est mieux servi — le plan est stocké et exportable — mais « pendant toute la durée des travaux » suppose de savoir que les travaux ont commencé, ce que `dateDebut` donne, sans qu'aucune surface le rapproche de l'exigence. Le blocage du 2° est celui des obligations événementielles : le déclencheur est l'ouverture des travaux~~, et le référentiel n'a pas d'axe pour ça~~ [2026-09-26 : rayé — la page « Quand ça arrive » existe (ADR-037) ; l'obligation n'y est pas encodée]. [2026-09-26 — annoncé à qui utilise le plan de prévention : quand `diagnostiquerPlan` conclut à l'écrit obligatoire, et seulement alors — c'est la condition que l'article pose —, l'article est cité entier dans la `recommandation` du diagnostic (lue par la carte du formulaire), dans une carte « Art. R. 4512-12 · pendant les travaux » de la fiche avec le constat « Rojer n'enregistre pas l'information prévue au 2°. », et sous chaque plan concerné du fichier `07_Plans_de_prevention.txt`. Aucun délai n'est affiché : l'article n'en fixe pas. Relu sur sa page propre le 2026-09-26 (structure demandée en aveugle, recopie, puis questions fermées sur la formulation décisive) : verbatim identique à celui du 2026-09-02. Rien n'est encodé : statut inchangé.]",
    },
    {
      ref: "R. 4512-13",
