...
| **E7** *(2026-09-26, lot « sans qualification »)* | **« Conforme » comme libellé du résultat d'un rapport** (`LABEL_RESULTAT`, badge du calendrier, registre PDF, tableau de bord, pastille « OK ») : c'est ce qu'écrit le vérificateur, affiché seul. L'écran a déjà remplacé « Non conforme » par « Écart relevé » ; le papier garde « Non conforme ». | Vocabulaire du produit : garder le mot du rapport (en le disant « selon le rapport du vérificateur »), ou l'aligner sur l'écran. |
| **E8** *(2026-09-26)* | **Effacement des titres d'un salarié (RGPD art. 17)** : `docs/rgpd.md` refuse l'effacement au titre d'une « obligation légale (preuve d'habilitation, `D. 4711-3`) » — exception de l'art. 17.3.b —, alors que le même document dit plus haut que `D. 4711-3` ne s'applique aux titres que « par analogie » (il vise les documents des vérifications et contrôles). Le texte montré au salarié (`salaries/droits.ts`) est corrigé le 2026-09-26 : il dit l'analogie. | Posture RGPD du produit : trouver le texte qui impose de CONSERVER les titres (s'il existe), ou fonder autrement le refus d'effacement (intérêt légitime, défense de droits en justice — art. 17.3.e), ou honorer l'effacement. |
| **E9** *(2026-09-26, lot permis de feu)* | **Mesures de la brochure INRS ED 6030 jamais proposées au permis de feu** : dégazage, contrôle d'atmosphère, bouteilles de gaz, supports incombustibles, issues, extinction des étincelles, refroidissement, colmatage. La liste corrigée compte 13 mesures, toutes tirées de la brochure ; l'étape « pendant » n'en porte plus qu'une. | Choix de contenu : compléter la liste avec ces lignes (même méthode, page par page), ou la garder courte. |
| **E10** *(2026-09-26, relecture des référentiels DUERP)* | **Textes copiés en base** : la description d'un risque et le libellé d'une mesure sont recopiés quand un dirigeant les retient. Corriger un référentiel ne change donc que les nouveaux DUERP ; six erreurs de fond ont été relevées (R. 4541-2/-3, seuil de R. 4541-9, contrôles électriques « annuels obligatoires », « train sur l'autoroute », R. 4421-1, repère de 10 kg). | Reprendre ou non les DUERP existants par une migration de données en production (les risques du référentiel ne sont pas modifiables par le dirigeant ; les libellés de mesures le sont peut-être). |
