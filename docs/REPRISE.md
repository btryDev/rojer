...
| `adr034-ligne-ouverte` | **en cours**, issue de `main` : l'ADR-034. N1, N2 et la moitié « à reprendre » de N4 faits le 2026-09-11 (la dernière réalisation se lit sur les rapports) ; **N3 fait le 2026-09-12** (l'archivage est le champ `archiveLe`, les prédicats ne lisent plus `dateRealisee`, migration `20260912090000_archivage_par_champ`) ; **N4 fait le 2026-09-13** (`etatDuRendezVous` supprimée, la lecture `prochaine` retirée, deux garanties du tableau de bord trouvées par mutation et tenues) ; reste N5. **Ne pas fusionner : la relecture neutre de N4 + `e450ea4` n'est pas jouée, et N5 n'est pas commencé.** La baisse du score, elle, est mesurée (2026-09-12, `pnpm mesure:score`) : rien pour un dossier à jour, jusqu'à −30 points pour un dossier dont trois échéances sont passées, −50 sur un parc partiellement retiré. Une échéance passée sur un appareil déjà contrôlé compte désormais en retard — c'est le défaut du lot 3 bis, et il valait 100 au lieu de 70. Base locale Docker (`duerp-a3-db`, port 5437 ; shadow 5438) dans le `.env` du worktree — jamais Supabase |
