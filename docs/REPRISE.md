...
| `adr034-ligne-ouverte` | **en cours**, issue de `main` : l'ADR-034. N1, N2 et la moitié « à reprendre » de N4 faits le 2026-09-11 (la dernière réalisation se lit sur les rapports) ; **N3 fait le 2026-09-12** (l'archivage est le champ `archiveLe`, les prédicats ne lisent plus `dateRealisee`, migration `20260912090000_archivage_par_champ`) ; restent la moitié « simplifiés » de N4, N5. **Ne pas fusionner avant N4 ni avant d'avoir mesuré la baisse du score** : une échéance passée sur un appareil déjà contrôlé compte désormais en retard. Base locale Docker (`duerp-a3-db`, port 5437 ; shadow 5438) dans le `.env` du worktree — jamais Supabase |
