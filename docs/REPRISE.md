...
| `adr034-ligne-ouverte` | **dans `main`** depuis le 2026-09-14, avec tous les lots qui l'ont suivie (retrait de `depassee`, lot C, titres, tableau de bord, placement sans échéance — `7f0f48d`). Historique : l'ADR-034. N1, N2 et la moitié « à reprendre » de N4 faits le 2026-09-11 (la dernière réalisation se lit sur les rapports) ; **N3 fait le 2026-09-12** (l'archivage est le champ `archiveLe`, les prédicats ne lisent plus `dateRealisee`, migration `20260912090000_archivage_par_champ`) ; **N4 fait le 2026-09-13** (`etatDuRendezVous` supprimée, la lecture `prochaine` retirée), **relu le même jour par deux sessions neutres et corrigé** : la règle « un statut réalisé ne purge que sans rendez-vous suivant » vit dans `retard.ts` (`estVerificationRealisee`), `periodicite` est requis sur `VerificationDatee`, `aDesarchiver` teste une clé obligation × porteur. 2618 tests, dix-huit mutations rouges. **Relu par un sous-agent externe** : « Conforme » peint à côté d'un retard (table `statutAffiche`), rangée jamais roulée lue en retard (`echeanceOuverte`, tranché par la propriétaire), dépôt refusé sur une ligne éteinte ; 2632 tests. **Seconde relecture externe corrigée** : l'échéance ouverte se calcule sur toute rangée réalisée périodique, comme le réconciliateur ; top 5, barres, tris et dépôt alignés ; 2643 tests. **N5 fait le 2026-09-13** : la propriétaire a tranché (données de prod fictives, on re-seed) — colonne `dateRealisee` retirée (migration `20260913120000_ligne_ouverte_retrait_date_realisee`, remise « planifiée » des rangées gelées puis `DROP COLUMN`, rejouée deux fois sur le Docker local), `echeanceOuverte` et toute la tolérance à l'ancienne rangée supprimées ; 2623 tests (les tests des rangées d'avant sont partis avec elles). **Fusionné et déployé le 2026-09-14** (`b8de22b`), après correction du build Vercel (échouait depuis N2 : `export class` dans un module `"use server"`). Pas de re-seed : la migration N5 a remis les données au modèle. **Retrait de `depassee`** : phase A fusionnée et déployée (`a7c80fd`) ; phase B fusionnée et déployée (`b4b2eb6`, migration `20260914120000_retrait_statut_depassee`). Relecture système GestBAT faite : aligné ; `lot/date-de-generation` (lot C) a corrigé l'affichage d'une date de génération comme échéance, fusionné. Ne rien fusionner sans le mot de la propriétaire. La baisse du score, elle, est mesurée (2026-09-12, `pnpm mesure:score`) : rien pour un dossier à jour, jusqu'à −30 points pour un dossier dont trois échéances sont passées, −50 sur un parc partiellement retiré. Une échéance passée sur un appareil déjà contrôlé compte désormais en retard — c'est le défaut du lot 3 bis, et il valait 100 au lieu de 70. Base locale Docker (`duerp-a3-db`, port 5437 ; shadow 5438) dans le `.env` du worktree — jamais Supabase |
